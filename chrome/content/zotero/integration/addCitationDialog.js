...
var Zotero_Citation_Dialog = new function () {
	...
	function load() {
		...
		ablocale = prefs.getBranch("general.useragent.").getCharPref("locale");
		ablocale=ablocale.substr(0,2);
		var abLocatorLabels=new Array();
		if ((ablocale=="fa") || (ablocale=="ar")) {
			abLocatorLabels=["صفحه","کتاب","فصل","ستون","تصویر","برگ",
						"شماره","خط","یادداشت","قطعه (opus)","پاراگراف","قطعه (part)","بخش","Sub verbo",
						"جلد","بیت"];
		} else {
			for (var value in locators) {
				var locator = locators[value];
				abLocatorLabels[value]=locator[0].toUpperCase()+locator.substr(1);
			}
		}
		//</abszh>
		...
		//<abszh>
		var menu2 = document.getElementById("label2");
		var label_list2 = document.getElementById("locator-type-popup2");
		//</abszh>
		var i = 0;
		for(var value in locators) {
			var locator = locators[value];
			...
			var locatorLabel=abLocatorLabels[value];
			//</abszh>
			//Zotero.Utilities.writeToDiagFile("locator is: "+locatorLabel+"\r\n");
			...
			//<abszh>
			var child2 = document.createElement("menuitem");
			child2.setAttribute("value", value);
			child2.setAttribute("label", locatorLabel);
			label_list2.appendChild(child2);
			//</abszh>
			// add to array
	...
	function _itemSelected(itemDataID, forceSave) {
		...
		for(var box in _preserveData) {
			...
			if(_lastSelected) {
				//<abszh>
				/*if(property == "label") {*/
				if((property == "label") || (property=="label2")) {
				//</abszh>
	...
	function _getCitation() {
		...
		if(_multipleSourcesOn) {
				...
				for(var i=0; i<listLength; i++) {
					...
					for (key in _itemData[itemDataID]) {
						...
						//<abszh>
							...
							citationItem.label2 = _locatorNameArray[_itemData[itemDataID].label2];
						//</abszh>
			...
			if(items.length) {
				...
				//<abszh>
				if(!citationItem["locator2"]) {
					delete citationItem["locator2"];
					delete citationItem["label2"];
				}
				
				if ((!citationItem["locator"]) && (citationItem["locator2"])) {
					citationItem["locator"]=citationItem["locator2"];
					citationItem["label"]=citationItem["label2"];
					delete citationItem["locator2"];
					delete citationItem["label2"];
				}
				//</abszh>
	...
	//<abszh>
	function abszhCorrectDirections() {
		var elem1=document.getElementById("remove");
		var elem2=document.getElementById("add");
		if (Zotero.Prefs.get('crcis.direction')=="rtl")	{
			if (elem1.disabled) {
				elem1.style.listStyleImage="url('chrome://zotero/skin/citation-add-gray.png')";
			} else {
				elem1.style.listStyleImage="url('chrome://zotero/skin/citation-add.png')";
			}
			if (elem2.disabled) {
				elem2.style.listStyleImage="url('chrome://zotero/skin/citation-delete-gray.png')";
			} else {
				elem2.style.listStyleImage="url('chrome://zotero/skin/citation-delete.png')";
			}
			
		} else {
			if (elem1.disabled) {
				elem1.style.listStyleImage="url('chrome://zotero/skin/citation-delete-gray.png')";
			} else {
				elem1.style.listStyleImage="url('chrome://zotero/skin/citation-delete.png')";
			}
			if (elem2.disabled) {
				elem2.style.listStyleImage="url('chrome://zotero/skin/citation-add-gray.png')";
			} else {
				elem2.style.listStyleImage="url('chrome://zotero/skin/citation-add.png')";
			}
		}		
