...
var CSL = {
	...
	CITE_FIELDS: ["first-reference-note-number", "locator"/*<abszh>*/,"locator2"/*</abszh>*/],	
	...
	NUMBER_REGEXP: /(?:^\d+|\d+$)/,
//<abszh> 
//added \u0600-\u06ff to some regexps 
//(replacing \u0400-\u052f with \u0400-\u052f\u0600-\u06ff)
//(replacing \u0400-\u042f with \u0400-\u042f\u0600-\u06ff)
//</abszh>
...
CSL.Engine = function (sys, style, lang, forceLang) {
	//Zotero.Utilities.writeToDiagFile("citeproce lang is: "+lang.toSource()+"\r\n");
...
CSL.Engine.prototype.getTerm = function (term, form, plural, gender, mode) {
	...
	//<abszh>
	if ("undefined" === typeof (this.opt.lang)) {
		this.opt.lang="en-US";
	}
	//</abszh>
...
CSL.Engine.prototype.fixOpt = function (token, name, localname) {
	//Zotero.Utilities.writeToDiagFile("token:"+token.name+"\r\n name:"+name+"\r\n localname:"+localname+"\r\n");
	...
	if ("name" === token.name || "names" === token.name) {
		...
		// if ("undefined" === typeof token.strings[localname]) {
			// Zotero.Utilities.writeToDiagFile("still undefined\r\n");
		// } else {
			// Zotero.Utilities.writeToDiagFile("this is it:"+token.strings[localname]+"\r\n");
		// }
...
CSL.Engine.prototype.localeConfigure = function (langspec) {
	var localexml;
	//Zotero.Utilities.writeToDiagFile("langspec is: "+langspec.toSource()+"\r\n");	
...
CSL.evaluateLabel = function (node, state, Item, item) {
	...
	/*<abszh>*/
	} else if ("locator2" === node.strings.term) {
		if (item && item.label2) {
			if (item.label2 === "sub verbo") {
				myterm = "sub-verbo";
			} else {
				myterm = item.label2;
			}
	...
	if ("number" !== typeof plural) {
		...
		//<abszh>
			...
			if (item) {
				plural = CSL.evaluateStringPluralism(item.locator2);				
			}
		//</abszh>
	...
	if ("label" === this.name && this.variables[0]) {
		...
		func = function (state, Item, item) {
			...
			for (pos = 0; pos < len; pos += 1) {
				...
				if (CSL.DATE_VARIABLES.indexOf(variable) > -1) {
					...
					break;
				/*<abszh>*/
				} else if ("locator2" === variable) {
					if (item && item.locator2) {
						output = true;
					}
					break;
				/*</abszh>*/
		...
		func = function (state, Item, item) {
			...
			for (pos = 0; pos < len; pos += 1) {
				...
				x = false;
				myitem = Item;
				if (item && [/*<abszh>*/"locator2",/*</abszh>*/"locator", "first-reference-note-number"].indexOf(variable) > -1) {
					myitem = item;
				}
				if (myitem[variable]) {
...
CSL.Parallel.prototype.isMid = function (variable) {
	return ["section", "volume", "container-title", "issue", "page", "page-first", "locator"/*<abszh>*/,"locator2"/*</abszh>*/].indexOf(variable) > -1;
};
