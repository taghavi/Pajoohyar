...
Zotero.Integration.Document.prototype._updateDocument = function(forceCitations, forceBibliography) {
	...
	for(var i in this._session.updateIndices) {
		...
		//<abszh>
		//Zotero.Utilities.writeToDiagFile("citation is: "+citation.toSource()+"\r\n");
		//if (citation.citationItems[0].item.language) {
			//Zotero.Utilities.writeToDiagFile("citation language is: "+citation.citationItems[0].item.language.toSource()+"\r\n");
		//} else {
			//Zotero.Utilities.writeToDiagFile("citation language is: undefined\r\n");
		//}
		//</abszh>
		
		//<abszh> converted if(citationText.indexOf("\\") !== -1) to if(true) in following line </abszh>
		//without this modification citation in vacouver style will appear as english numbers sometimes.
		if(true) {
			// need to set text as RTF
			//<abszh>
			var ablocale=Zotero.Prefs.get('crcis.citationLanguage');
			var makeItRtl=false;
			var tempItem=citation.citationItems[0].item;
			//Zotero.Utilities.writeToDiagFile("citationLanguage is : "+ablocale+"\r\n");
			//Zotero.Utilities.writeToDiagFile("styleClass is : "+this._session.styleClass.toSource()+"\r\n");
			
			if (((ablocale=="fa") || (ablocale=="ar")) && (this._session.styleClass == "in-text")) {
				makeItRtl=true;
			} else if (Zotero.Utilities.checkLang(tempItem.language,tempItem.title) &&  (this._session.styleClass == "note")) {
				makeItRtl=true;
			}
			
			if (makeItRtl) {
				citationText=citationText.replace(/,/g,"{\\uc0\\u1548}");	//replace , with ،
				citationText=citationText.replace(/;/g,"{\\uc0\\u1563}");  	//replace ; with ؛
				//Zotero.Utilities.writeToDiagFile("citationText is: "+citationText.toSource()+"\r\n");
				this._fields[i].setText("{\\rtf\\rtlch "+citationText+"}", true);
			} else {
			//</abszh>
	...
	if(this._bibliographyFields.length	 				// if blbliography exists
		...
		if(bib) {
			...
			if (!sepEnable) {
				...
				//Zotero.Utilities.writeToDiagFile("bibliographyTextArray[0]: "+bibliographyTextArray[0]+"\r\n");
				//Zotero.Utilities.writeToDiagFile("bibliographyTextArray[1]: "+bibliographyTextArray[1]+"\r\n");
				//Zotero.Utilities.writeToDiagFile("bibliographyTextArray[2]: "+bibliographyTextArray[2]+"\r\n");
				//Zotero.Utilities.writeToDiagFile("firstBibLanguages: "+firstBibLanguages.toSource()+"\r\n");
				//Zotero.Utilities.writeToDiagFile("secondBibLanguages: "+secondBibLanguages.toSource()+"\r\n");

				bibliographyText=bib[0].bibstart+bibliographyTextArray[0]+bibliographyTextArray[1]+bibliographyTextArray[2]+"\\\r\n"+bib[0].bibend;
			}
			//</abszh>
			
			//<abszh> commented out the following line
			//bibliographyText = bib[0].bibstart+bib[1].join("\\\r\n")+"\\\r\n"+bib[0].bibend;

			//<abszh>
			//Zotero.Utilities.writeToDiagFile(bibliographyText+"\r\n");			
			//</abszh>
...
Zotero.Integration.Session.prototype.getCitationField = function(citation) {
	...
	const saveCitationItems = ["locator", "label", "locator2","label2","suppress-author", "author-only", "prefix", "suffix", "uri"];
	//</abszh>
...
Zotero.Integration.Session.prototype.getBibliography = function() {
	...
	//<abszh>
	//var langspec={base:"fa-FA", best:"fa-FA", bare:"fa"};
	//this.style.localeConfigure(langspec);
	//Zotero.Utilities.writeToDiagFile("abszh generated langspec: "+langspec.toSource());
	//</abszh>
	// generate bibliography
	//<abszh>
	var langspec={base:"en-US", best:"en-US", bare:"en"};
	this.style.localeConfigure(langspec);
	//</abszh>
	...
	//<abszh>
	var lang=Zotero.Prefs.get('crcis.citationLanguage');
	langspec.base=lang+"-"+lang.toUpperCase;
	langspec.best=langspec.base;
	langspec.bare=lang;
	this.style.localeConfigure(langspec);
	//</abszh>
	...
	if(bib) {
		...
		//<abszh>
		//find all cited items with rtlBibliography checked
		var rtlItems=[];
		for (var i in this.citationsByIndex)
		{
			//this array will contain all citations in a multi source citation
			var abCitationItems=this.citationsByIndex[i].citationItems; 
			//loop through every single citation
			for (var j in abCitationItems)
			{
				if (abCitationItems[j].rtlBibliography)
				{
					if (abCitationItems[j].id) {
						rtlItems[abCitationItems[j].id]=true;
						//Zotero.Utilities.writeToDiagFile("this is an rtlBib item:"+abCitationItems[j].id+"\r\n");
					}
				}
			}
		}
		...
		for(var i in bib[0].entry_ids) {
			...
			if (Zotero.Utilities.checkLang(abItem.language,abItem.title)) {
					//Zotero.Utilities.writeToDiagFile("it is a persian or arabic item\r\n");
					bib[1][i]=bib[1][i].replace(/,/g,"{\\uc0\\u1548}");	//replace , with ،
					bib[1][i]=bib[1][i].replace(/;/g,"{\\uc0\\u1563}");  	//replace ; with ؛
					bib[1][i]="\\rtlpar\\qr "+bib[1][i];
					//Zotero.Utilities.writeToDiagFile("bib[1][i] is: "+bib[1][i].toSource()+"\r\n");
					//Zotero.Utilities.writeToDiagFile(Zotero.Utilities.inspectObject(bib[1][i]+"\r\n"));
			} else {
					//Zotero.Utilities.writeToDiagFile("it is NOT a persian nor arabic item\r\n");
					bib[1][i]="\\ltrpar\\ql "+bib[1][i];			
			}				
			//</abszh>
			//<abszh>
				//if (this.citationsByIndex) 
			//Zotero.Utilities.writeToDiagFile(Zotero.Utilities.inspectObject(this.citationsByIndex,10,0)+"\r\n")
				//var abCitations=this.citationsByItemId[bib[0].entry_ids[i]];
				//Zotero.Utilities.writeToDiagFile(bib[0].entry_ids[i]+","+abCitations.rtlCitation+"\r\n");
			//</abszh>
		}
		//<abszh>
		//sorting by language
...
Zotero.Integration.Session.prototype.formatCitation = function(index, citation) {
	if(!this.citationText[index]) {
		...
		//<abszh>
		//Zotero.Utilities.writeToDiagFile("Integration: style.processCitationCluster("+citation.toSource()+", "+citationsPre.toSource()+", "+citationsPost.toSource()+"\r\n");
		//</abszh>
		//<abszh>
		//var oldLabel=citation.citationItems[0].label;
		//var oldLocator=citation.citationItems[0].locator;
		//if (oldLabel) Zotero.Utilities.writeToDiagFile("oldLabel is"+oldLabel.toSource())
		//if (oldLocator) Zotero.Utilities.writeToDiagFile("oldLabel is"+oldLocator.toSource())
		//citation.citationItems[0].label="chapter";
		//citation.citationItems[0].locator="1024";
...
*/		
		/*
		var anyLocator2=false;
		var memLocators=[];
		var memLabels=[];
		var locator2Indexes=[];
		for (var i=0; i<citation.citationItems.length; i++) {
			if (citation.citationItems[i].locator2) {
				//Zotero.Utilities.writeToDiagFile("citation.citationItems[i] is:"+citation.citationItems[i].toSource()+"\r\n");
				anyLocator2=true;
				locator2Indexes[i]=true;
				memLabels[i]=citation.citationItems[i].label;
				memLocators[i]=citation.citationItems[i].locator;				
				citation.citationItems[i].label=citation.citationItems[i].label2;
				citation.citationItems[i].locator=citation.citationItems[i].locator2;
			}
		}
		if (anyLocator2) {
			var newCitations2 = this.style.processCitationCluster(citation, citationsPre, citationsPost);
			for (i in locator2Indexes) {
				citation.citationItems[i].locator=memLocators[i];
				citation.citationItems[i].label=memLabels[i];
			}
		}
		//</abszh>
		*/
		...
		for each(var newCitation in newCitations[1]) {
			...
			//<abszh>
			//Zotero.Utilities.writeToDiagFile("result in formatCitation in integration.js: "+newCitation[0]+","+newCitation[1]+"\r\n");
			//</abszh>
		}

		//<abszh>
		/*
		for (var i=0; i<newCitations[1].length; i++) {
			if (locator2Indexes[i]) {
				var str1=""+newCitations[1][i][1];
				var str2=""+newCitations2[1][i][1];
				var minLength=str1.length;
				if (str2.length<minLength) minLength=str2.length;
				var diffPoint=minLength;
				...
				var dif1="";
				var dif2="";
				if (diffPoint<minLength) {
					commonPart=str1.substr(0,diffPoint);
					dif1=str1.substr(diffPoint);
					dif2=str2.substr(diffPoint);
				} 
				this.citationText[citationIndices[newCitations[1][i][0]]]=commonPart+dif1+dif2;
			} else {
				this.citationText[citationIndices[newCitations[1][i][0]]] = newCitations[1][i][1];
			}
			this.updateIndices[citationIndices[newCitations[1][i][0]]] = true;
		}
		*/
		//</abszh>
