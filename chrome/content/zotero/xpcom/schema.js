...
Zotero.Schema = new function(){
	...
	this.updateFromRepository = function (force, callback) {
		...
		var url = ZOTERO_CONFIG['REPOSITORY_URL'] + '/updated.php?'
			 + (lastUpdated ? 'last=' + lastUpdated + '&' : '')
			 + 'version=' + Zotero.version;
		//</abszh>
	...
	function _initializeSchema(){
		...
		try {
			...
			if (!Zotero.Schema.skipDefaultData) {
				...
				Zotero.DB.query(sql);
				var sql = "INSERT INTO itemDataValues VALUES (2, 'http://pajoohyar.ir')";
				Zotero.DB.query(sql);
				var sql = "INSERT INTO itemData VALUES (1, 1, 2)";
				Zotero.DB.query(sql);
				
				// CHNM as creator
				var sql = "INSERT INTO creatorData VALUES (1, '', 'Pajoohyar', '', 1, NULL)";
				Zotero.DB.query(sql);
